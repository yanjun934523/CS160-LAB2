...
void eval(char *cmdline) 
{
    ...
    //if it is not build-in cmd
      ...
      pid = fork();
      if(pid == 0){
      //this only the child process enters in to thi branch
         sigprocmask(SIG_UNBLOCK, &mask, NULL);//a child first unblock th signals
	 setpgid(0, 0);//set a  unique id for all new process, such that a SIGINT will not kill all the processes
	 if(execve(argv[0], argv, environ) < 0){
	 //it is possbile that we type in an illegal cmd
	    printf("Not a valid command.\n");
	    exit(0); 
	}    
      }
    //the signals could run again
    addjob(jobs, pid, (bg == 1 ? BG : FG ), cmdline );
    sigprocmask(SIG_UNBLOCK, &mask, NULL);
    //if the job is running in FG, wait for it to terminate and then return
    if(!bg) waitfg(pid);
...
int builtin_cmd(char **argv) 
{
    char *cmd = argv[0];
    if(!strcmp(cmd, "jobs")){
    	listjobs(jobs);//if input is jobs
	return 1;
    }
    if(!strcmp(cmd, "quit")){
         exit(0);//if input is quit
     }
    if(!strcmp(cmd, "bg")|| (!strcmp(cmd,"fg"))){
        do_bgfg(argv) ;//if input is bg or fg
	return 2;
     }
    if(!strcmp(cmd, "&")){
	return 3;
...
void do_bgfg(char **argv) 
{
    ...
    //distinguish two cases:1bg/fg + PID/%JID
    if(isdigit(argv[1][0])){
    //PID case
	   ...
	   if(argv[1][0] == '%'){
	      ...
	      //if(jobp is invalid) printf("we dont have such a job in the job list.\n)
	      //return
	      if(jobp == NULL) {
    ...
    //we made sure that all inputs arrive this point are valid
    //if this is  a bg cmd
    if(!strcmp(argv[0], "bg")){
       pid_t pid = jobp->pid;
       kill(-pid, SIGCONT);
...
void sigchld_handler(int sig) 
{
    ...
    while ( (child_pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0 ){
	     //We have detected a terminater or a stopped job-- we dont wait for others during which
